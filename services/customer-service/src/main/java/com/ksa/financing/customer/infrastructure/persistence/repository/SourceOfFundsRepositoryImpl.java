...
public class SourceOfFundsRepositoryImpl implements SourceOfFundsRepository {
    ...
    @Override
    public PageResponse<SourceOfFundsOption> findAllByTenantId(UUID tenantId, PageQuery pageQuery) {
        log.debug("Finding all source of funds options for tenant: {} with query: {}", tenantId, pageQuery);
    ...
    @Override
    public PageResponse<SourceOfFundsOption> findActiveByTenantId(UUID tenantId, PageQuery pageQuery) {
        log.debug("Finding active source of funds options for tenant: {} with query: {}", tenantId, pageQuery);
        var page = jpaRepository.findAll(buildSpec(tenantId, true, pageQuery), pageQuery.toPageable());
        return PageResponse.from(page, ReferenceDataPersistenceMapper::toDomain);
    }

    private Specification<SourceOfFundsOptionJpaEntity> buildSpec(UUID tenantId, boolean activeOnly, PageQuery pageQuery) {
        Specification<SourceOfFundsOptionJpaEntity> base = (root, q, cb) -> cb.and(
        ...
        Specification<SourceOfFundsOptionJpaEntity> dynamic = SpecificationBuilder.<SourceOfFundsOptionJpaEntity>builder()
