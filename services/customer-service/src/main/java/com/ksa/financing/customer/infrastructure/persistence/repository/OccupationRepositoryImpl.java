...
public class OccupationRepositoryImpl implements OccupationRepository {
    ...
    private final JpaOccupationRepository jpaRepository;

    @Override
    public OccupationOption save(OccupationOption option) {
        var entity = ReferenceDataPersistenceMapper.toOccupationEntity(option);
        var saved = jpaRepository.save(entity);
        return ReferenceDataPersistenceMapper.toDomain(saved);
    }

    @Override
    public Optional<OccupationOption> findById(UUID tenantId, UUID id) {
        return jpaRepository.findByIdAndTenantIdAndDeletedFalse(id, tenantId)
                .map(ReferenceDataPersistenceMapper::toDomain);
    }

    @Override
    public PageResponse<OccupationOption> findAllByTenantId(UUID tenantId, PageQuery pageQuery) {
        log.debug("Finding all occupation options for tenant: {} with query: {}", tenantId, pageQuery);
    ...
    @Override
    public PageResponse<OccupationOption> findActiveByTenantId(UUID tenantId, PageQuery pageQuery) {
        log.debug("Finding active occupation options for tenant: {} with query: {}", tenantId, pageQuery);
        var page = jpaRepository.findAll(buildSpec(tenantId, true, pageQuery), pageQuery.toPageable());
        return PageResponse.from(page, ReferenceDataPersistenceMapper::toDomain);
    }

    private Specification<OccupationOptionJpaEntity> buildSpec(UUID tenantId, boolean activeOnly, PageQuery pageQuery) {
        Specification<OccupationOptionJpaEntity> base = (root, q, cb) -> cb.and(
        ...
        Specification<OccupationOptionJpaEntity> dynamic = SpecificationBuilder.<OccupationOptionJpaEntity>builder()
