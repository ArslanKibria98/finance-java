...
public class PurposeOfFinanceRepositoryImpl implements PurposeOfFinanceRepository {
    ...
    @Override
    public PageResponse<PurposeOfFinanceOption> findAllByTenantId(UUID tenantId, PageQuery pageQuery) {
        log.debug("Finding all purpose of finance options for tenant: {} with query: {}", tenantId, pageQuery);
    ...
    @Override
    public PageResponse<PurposeOfFinanceOption> findActiveByTenantId(UUID tenantId, PageQuery pageQuery) {
        log.debug("Finding active purpose of finance options for tenant: {} with query: {}", tenantId, pageQuery);
        var page = jpaRepository.findAll(buildSpec(tenantId, true, pageQuery), pageQuery.toPageable());
        return PageResponse.from(page, ReferenceDataPersistenceMapper::toDomain);
    }

    private Specification<PurposeOfFinanceOptionJpaEntity> buildSpec(UUID tenantId, boolean activeOnly, PageQuery pageQuery) {
        Specification<PurposeOfFinanceOptionJpaEntity> base = (root, q, cb) -> cb.and(
        ...
        Specification<PurposeOfFinanceOptionJpaEntity> dynamic = SpecificationBuilder.<PurposeOfFinanceOptionJpaEntity>builder()
