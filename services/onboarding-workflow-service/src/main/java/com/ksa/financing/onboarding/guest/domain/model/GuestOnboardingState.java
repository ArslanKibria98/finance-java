...
public class GuestOnboardingState implements Serializable {

    private static final long serialVersionUID = 1L;

    private GuestOnboardingStep currentStep;
    ...
    /**
     * Flag that tells downstream systems this user finished only the lightweight guest
     * flow and has NOT yet completed full KYC onboarding (document + selfie + profile).
     * Defaults to {@code false} for every guest workflow. Flipped to {@code true} only
     * when the user later completes the full onboarding flow.
     */
    ...
    public GuestOnboardingState() {
        this.onboardingComplete = false;
    }

    public GuestOnboardingStep getCurrentStep() { return currentStep; }
    public void setCurrentStep(GuestOnboardingStep currentStep) { this.currentStep = currentStep; }
